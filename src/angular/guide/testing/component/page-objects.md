...
The `app_po.g.dart` file is generated by the pageloader builder
...
Create the following `@PageObject()` annotated abstract class named `AppPO`
containing a `part` statement and a factory constructor:
...
A PO is only useful if it has fields. You'll be declaring those shortly.

During the build process, pageloader generates an implementation for your PO
class based on the fields you declare, and saves the implementation to
`app_po.g.dart`. The factory method `$AppPO.create` comes from the generated
code.
